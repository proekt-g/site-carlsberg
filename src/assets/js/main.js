...
window.addEventListener("load", function () {
  let pointSvgPolygon;
  let widthSlide, heightCloseSlide, heightSlide;
  ...
  $.fn.rotatingSlider = function (options) {
    var rotatingSlider = {
      ...
      handleDragMove: function (e) {
        if (this.readyToDrag) {
          ...
          if (this.currentlyDragging) {
            ...
            this.$slidesContainer.css("-webkit-transform", "translateX(-50%) rotate(" + -this.currentRotationAngle + "deg)");
            this.$slidesContainer.css("-ms-transform", "translateX(-50%) rotate(" + -this.currentRotationAngle + "deg)");
          }
        }
      },
      ...
      renderSlider: function () {
        ...
        this.$slidesContainer.css("-webkit-transform", "translateX(-50%)");
        this.$slidesContainer.css("-ms-transform", "translateX(-50%)");
        ...
        this.$slides.each(
          function (i, el) {
            ...
            $slide.css("-ms-transform-origin", "center " + (innerRadius + this.settings.slideHeight) + "px");
            $slide.css("-webkit-transform-origin", "center " + (innerRadius + this.settings.slideHeight) + "px");
            $slide.css("height", this.settings.slideHeight + "px");
            $slide.css("width", this.settings.slideWidth + "px");
            $slide.css(
              "padding",
              upperArcHeight * this.settings.mePaddingCoef +
                "px " +
                slideSidePadding * this.settings.mePaddingCoef +
                "px " +
                lowerArcHeight * this.settings.mePaddingCoef +
                "px " +
                slideSidePadding * this.settings.mePaddingCoef +
                "px "
            );
            ...
            $slide.css("-webkit-transform", "translateX(-50%) rotate(" + -this.slideAngle * i + "deg) translateY(-" + upperArcHeight + "px)");
            $slide.css("-ms-transform", "translateX(-50%) rotate(" + -this.slideAngle * i + "deg) translateY(-" + upperArcHeight + "px)");
            let nameSvg;
            this.$slider[0].classList.forEach(function (item) {
              if (item === "rotating-slider--main") {
                nameSvg = "slideClipPath";
                var NUM_POINTS = 180;
                let strCustom = "polygon(";
                let pathCustom = document.getElementById("slideClipPath").querySelector("path");
                let lenCustom = pathCustom.getTotalLength();
                let pointsCustom = [];
                setTimeout(() => {
                  for (var i = 0; i < NUM_POINTS; i++) {
                    // console.log(i * i * i);
                    var pt = pathCustom.getPointAtLength((i * lenCustom) / (NUM_POINTS - 1));
                    pointsCustom.push([pt.x, pt.y]);
                    strCustom += pt.x;
                    strCustom += "px ";
                    strCustom += pt.y;
                    strCustom += i !== NUM_POINTS - 1 ? "px, " : "px)";
                  }
                  pointSvgPolygon = strCustom;

                  $slide.css("-webkit-clip-path", strCustom);
                  $slide.css("clip-path", strCustom);
                }, 1000);
              }
  ...
  function addModifierElement(_element, master = false, modifier) {
    if (master)
      document.querySelectorAll(`.${_element.classList[0]}--${modifier}`).forEach(function (item) {
        item.classList.toggle(`${_element.classList[0]}--${modifier}`);
      });
    if (Array.isArray(_element) || _element.length !== undefined)
  ...
  $slide.forEach(function (item, index) {
    ...
    if (item.closest(".rotating-slider").querySelectorAll(".rotating-slider")[index].classList.contains("rotating-slider__junuary"))
      _className = ".rotating-slider__junuary";
    ...
    if (item.closest(".rotating-slider").querySelectorAll(".rotating-slider")[index].classList.contains("rotating-slider__march"))
      _className = ".rotating-slider__march";
    ...
    if (item.closest(".rotating-slider").querySelectorAll(".rotating-slider")[index].classList.contains("rotating-slider__may"))
      _className = ".rotating-slider__may";
    ...
    if (item.closest(".rotating-slider").querySelectorAll(".rotating-slider")[index].classList.contains("rotating-slider__july"))
      _className = ".rotating-slider__july";
    if (item.closest(".rotating-slider").querySelectorAll(".rotating-slider")[index].classList.contains("rotating-slider__august"))
      _className = ".rotating-slider__august";
    ...
    if (item.closest(".rotating-slider").querySelectorAll(".rotating-slider")[index].classList.contains("rotating-slider__october"))
      _className = ".rotating-slider__october";
    ...
    if (item.closest(".rotating-slider").querySelectorAll(".rotating-slider")[index].classList.contains("rotating-slider__december"))
      _className = ".rotating-slider__december";
    item.addEventListener("click", addModifierElement.bind(null, document.querySelector(_className), true, "active"), false);
    item.addEventListener(
      "click",
      createRotatingSlider.bind(
        null,
        _className,
        false,
        2000,
    ...
    item.addEventListener(
      "click",
      addModifierElement.bind(null, document.querySelector(_className).querySelectorAll(".slide--day"), 0, "active"),
      false
    );
  ...
  function hoverSliderElement(index, width = widthSlide, height = heightSlide, svgName = "slideClipPathMe", coef = 17) {
    ...
    $slide.css("-webkit-transform", "translateX(-50%) rotate(" + -this.slideAngle * index + "deg) translateY(-" + upperArcHeight + "px)");
    $slide.css("-ms-transform", "translateX(-50%) rotate(" + -this.slideAngle * index + "deg) translateY(-" + upperArcHeight + "px)");
    var NUM_POINTS = 180;
    let strCustom = "polygon(";
    let pathCustom = document.getElementById(svgName).querySelector("path");
    let lenCustom = pathCustom.getTotalLength();
    let pointsCustom = [];
    for (var i = 0; i < NUM_POINTS; i++) {
      var pt = pathCustom.getPointAtLength((i * lenCustom) / (NUM_POINTS - 1));
